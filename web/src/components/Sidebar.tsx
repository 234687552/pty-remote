...
import type { ManagedPtyLifecycle, ProjectSessionSummary } from '@shared/protocol.ts';
  ...
  async function handlePrimaryAction(): Promise<void> {
    ...
    try {
      if (activeProject) {
        ...
        const cwdInput = window.prompt('请输入要添加的目录路径');
                ...
                <div className="relative overflow-hidden rounded-xl">
                  ...
                  >
                    <button
                      ...
                      onClick={() => {
                        if (shouldBlockRowAction(projectSwipeRowKey)) {
                          return;
                        }
      ...
      <div className="border-t border-zinc-200 bg-white/95 px-3 py-3 backdrop-blur-sm">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex h-10 w-10 items-center justify-center rounded-xl bg-zinc-900 text-white transition hover:bg-zinc-700 disabled:cursor-not-allowed disabled:opacity-50"
            aria-label={
              createConversationPending ? (activeProject ? '创建会话中' : '添加目录中') : activeProject ? '创建会话' : '添加目录'
            }
            title={
              createConversationPending ? (activeProject ? '创建会话中' : '添加目录中') : activeProject ? '创建会话' : '添加目录'
            }
            disabled={!createProviderId || createConversationPending}
          >
            <svg viewBox="0 0 20 20" fill="none" aria-hidden="true" className={['h-4 w-4', createConversationPending ? 'animate-pulse' : ''].join(' ')}>
